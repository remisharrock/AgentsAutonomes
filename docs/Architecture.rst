...
:Author: Marie-Laure Bardonner
:Author: Giulia Zago
:Author: Marc Nasrallah
:Author: 胡雨軒
...
La version la plus à jour de ce document est originellement composée
avec LaTeX(au format ``tex``). Deux formats de sortie sont proposés :
``pdf`` et ``md`` pour s’adapter à une lecture directement depuis GitHub
ou hors ligne. La version
...
en quelque sorte des réalisations  [1]_ dans le niveau inférieur.

Canal
'''''

Un canal (``models.Channel`` dans le code) est une catégorie d’éléments
de l’internet des objets  [2]_ : ce peut être une classe d’objets
physiques ou immatériels comme un capteur, une lampe, le compte d’un
service en ligne, un site de news. Comme dans ifttt, des canaux ont des
signaux (triggers en anglais et ``models.Trigger`` dans le code) et des
actions.
...
d’état  [3]_. Il peut mentionner des modalités (dans le cas d’une porte,
...
définit (``actors.Commutator`` dans le code) pour permettre aux acteurs
de lever des signaux simplement en envoyant un message à icelui. C’est
...
secondes) ou plus évoluée : lois de Bernouilli, Gauss, Poisson\ …
...
Tordons le cou à une fausse idée : ``Commutator`` utilise une
réalisation de l’interface ``LookUpEventBus``. Cette interface
fonctionne avec un patron éditeur / lecteur (publisher / subscriber en
anglais). En revanche, le commutateur porte bien son nom puisse qu’il a
pour effet d’établir comme une ligne téléphonique entre deux acteurs. De
la même manière que dans la boucle locale, il n’y a pas :math:`n^2`
lignes pour relier n utilisateurs du téléléphone mais que la commutation
par circuit donne cette illusion, l’objet ``Commutator`` émule cette
situation : dans le cas d’une relation de causalité strictement
bijective, il ne peut y avoir qu’un destinateur et un destinataire. En
revanche, le patron éditeur / lecteur est effectif si plusieurs
relations de causalité prennent pour source la même classe de message et
le même acteur.
...
-  La manière la plus simple est de commander une action, donc d’envoyer
   un message d’action à un acteur ;
...
La classe ``RandomScheduler`` propose les deux premières façons de
faire. La troisième est laissée au soin du lecteur.
...
élément du second. On rappelle qu’une relation dans un ensemble
:math:`E` est caractérisée par un sous-ensemble du produit cartésien
:math:`E × E`, soit une collection de doublet d’éléments de :math:`E`.

Définitions
-----------

Soient :math:`S` et :math:`C` respectivement les ensembles des signaux
et des canaux. L’ensemble :math:`S` se décompose en deux parties
exclusives :math:`S_E` et :math:`S_R` car un signal est soit émis (le
premier) soit reçu (le second). Un canal peut avoir plusieurs signaux.
Un canal sans signal est comme une soupe sans sel : c’est moins bon
puisqu’il ne peut pas communiquer.

Une recette de rang :math:`(m, n)` est une relation de
:math:`\left(S×C\right)^m` dans :math:`\left(S×C\right)^m` qui lie m
doublets à n autre doublets. Une recette dont le rang n’est pas récisé
est une recette de rang :math:`(1, 1)`. Une recette est dite réalisable
si et seulement si :

-  Pour tout doublet (s, c) de :math:`\left(S×C\right)^m`, :math:`s` est
   élément de :math:`E` (c’est un signal émis) ;

-  Pour tout doublet (s, c) de :math:`\left(S×C\right)^n`, :math:`s` est
   élément de :math:`R` (c’est un signal reçu) ;

Pour définir clairement les choses, une recette (de rang :math:`(1, 1)`)
est une application dans :math:`S×C` qui lie un doublet
:math:`d_1 = (e, c_1)` à un autre doublet :math:`d_2 = (r, c_2)`. Une
telle recette est dite réalisable si et seulement si :math:`e`
appartient à :math:`E` et :math:`r` à :math:`R`. On notera incidemment
que les canaux impliqués dans une recette ne sont, suivant cette
définition, pas forcément, tous différents.

D’autre part, soit :math:`M` et :math:`A` respectivement l’ensemble des
espaces de messages et l’ensemble des espaces acteurs. Tout élément de
:math:`A` est un acteur, qui peut envoyer et recevoir des messages. Tout
élément de l’ensemble :math:`M` des espaces des message appartient
exclusivement à l’un des deux sous-ensembles :math:`M_E` et :math:`M_A`
car un message contient une sémantique particulière : il est envoyé par
un acteur après un évènement ou reçu par un acteur pour accomplir une
action.
...
Une relation de causalité de rang :math:`(m, n)` est définie sur
:math:`\left(M×A\right)^m` dans :math:`\left(M×A\right)^m` et lie
:math:`m` doublets d’émission à :math:`n` doublets de réception . Une
relation de causalité dont le rang n’est pas précisé est de rang
:math:`(1, 1)`. Par un abus de langage bien pratique, on définit les
types de rang suivants :

-  Une relation de causalité de rang :math:`(m, n), m > n` est une
   injection, dite stricte quand :math:`n = 1` ;

-  Une relation de causalité de rang :math:`(m, n), n > m` est une
   surjection, dite stricte quand :math:`m = 1` ;

-  Une relation de causalité de rang :math:`(m, n), m = n` est une
   bijection, dite stricte quand :math:`m = n = 1`.
...
arguments sont m messages de classes éléments de :math:`M` et produit
comme résultat au plus n messages sur l’ensemble d’arrivée. Cela veut
dire que la fonction reçoit un message de chaque acteur de départ liés
par la relation de causalité mais, en fonction de ses entrées, n’envoit
pas forcément un message à tous les acteurs d’arrivée de la relation.
...
délai court à partir du premier message sur :math:`m` émis par un acteur
et les :math:`m - 1` messages suivants doivent être reçus au plus tard
...
de rang :math:`(1, 1)` qui sont dont des bijections strictes. Le délai
de traitement n’est donc pas utile et n’est pas considéré.
...
-  On attend tout d’abord dans ce projet qu’un acteur puisse être lié
   par des relations de causalité : serait-il acceptable qu’une relation
   de causalité lie les acteurs \`MessageMap\` ou \`Commutator\`, qui
   sont utilisés pour caractériser justement ces relations ?
...
de développement kiss : *keep it simple stupid*. Le nom de la branche
vient de là.
...
cycle sur le graphe :math:`(A, C)` des acteurs et des relations de
causalité. La mise en œuvre est en revanche hardue : car en réalité ce
n’est pas un graphe (mais ça se dessine pareil donc la structure est
pareille) et il ne faut pas oublier que cet chose pseudo-graphe joue
avec des pointeurs de fonction, des pointeurs de classe et des acteurs.
Je suis très currieux de voir l’algorithme de recherche de cycle sur un
hyper-graphe qui correspond au cas général des relations de causalité de
rang quelconque.

Entre commutateurs
^^^^^^^^^^^^^^^^^^

:math:`x` acteurs touchés par des relations de causalité qui
appartiennent à :math:`y` commutateurs différents, :math:`x` et
:math:`y` quelconques.

Externe au commutateur
^^^^^^^^^^^^^^^^^^^^^^

Par exemple l’influence thermodynamique mutuelle de :math:`x` acteurs
radiateurs dont les relations de causalité respectives sont émulées par
:math:`y, y > x` commutateurs sans qu’un commutateur émule deux recettes
du même radiateur.
...
.. [1]
   Au sens philosophique du terme : rendre réel ; synonyme de l’anglais
   « to implement ».

.. [2]
   Plus précisément : une catégorie d’objets de l’internet

.. [3]
   Par exemple : *la porte est ouverte* pour un capteur de porte ou bien
   *quelque chose a bougé* pour un détecteur de mouvement
